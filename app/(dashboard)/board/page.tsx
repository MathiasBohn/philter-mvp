...
import { Search, Eye, TrendingUp, Clock, CheckCircle } from "lucide-react";
...
export default function BoardDashboardPage() {
  ...
  const boardApplications = mockApplications.filter(
    (app) => app.status === ApplicationStatus.IN_REVIEW || app.status === ApplicationStatus.SUBMITTED
  );
  ...
  const reviewedCount = boardApplications.filter(
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold">Board Review Dashboard</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{boardApplications.length}</div>
            <p className="text-xs text-muted-foreground">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <CardDescription className="mt-1">
                        <span className="font-medium">{app.building?.name || "Unknown Building"}</span>
